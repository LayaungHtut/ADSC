...
Writes into `src/lib/data/`:
  - risk.json        : township risk + geometry (GeoJSON, compact)
  ...
  - district.json    : district-level summary
"""
...
from floodresilience.config import DATA_PROCESSED, DATA_INTERMEDIATE, DOCUMENTED_YANGON_FLOOD_YEARS
...
def main() -> None:
    ...
    gdf = gpd.read_file(DATA_PROCESSED / "yangon_township_risk.geojson")
    gdf = gdf.to_crs(4326)
    gdf["geometry"] = gdf.geometry.simplify(tolerance=0.0001, preserve_topology=True)
    geojson = json.loads(gdf.to_json())
    (WEB_DATA / "risk.json").write_text(json.dumps(geojson, separators=(",", ":")), encoding="utf-8")

    features = pd.read_csv(DATA_PROCESSED / "yangon_township_features.csv")
    ...
    rain["flood_year"] = rain["year"].isin(DOCUMENTED_YANGON_FLOOD_YEARS).astype(int)
    (WEB_DATA / "rainfall.json").write_text(rain.to_json(orient="records"), encoding="utf-8")

    district = features.groupby(["district", "district_code"]).agg(
        n_township=("tship_code", "count"),
        pop_est=("pop_est", "sum"),
        schools=("schools", "sum"),
        health_facilities=("health_facilities", "sum"),
    ).reset_index()
    district = district.round(1)
    (WEB_DATA / "district.json").write_text(district.to_json(orient="records"), encoding="utf-8")
