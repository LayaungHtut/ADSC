...
Windowed reads fetch only the needed bytes (range requests), so no large
...
def fetch_month(year: int, month: int, out_dir: Path = OUT_DIR) -> Path | None:
    """Window-read one global COG clipped to the Yangon bbox. Returns output path or None."""
