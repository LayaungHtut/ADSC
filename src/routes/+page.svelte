<script lang="ts">
	...
	const top = topRisk[0];
	const highest = top ? (byCode.get(top.kec_code) ?? null) : null;
</script>

<svelte:head>
	<title>Dashboard — FloodResilience Jakarta</title>
</svelte:head>

<section class="mb-8">
	<h1 class="text-3xl font-bold tracking-tight">Flood risk intelligence for Jakarta</h1>
	<p class="mt-2 max-w-3xl text-slate-600">
		A data-driven assessment of flood risk across Jakarta's 42 urban kecamatan, combining rainfall,
		elevation, population and critical infrastructure. Built entirely from public, traceable data.
	</p>
</section>

<section class="grid grid-cols-2 gap-4 lg:grid-cols-4">
	<KpiCard
		label="Kecamatan assessed"
		value={String(riskSummary.length)}
		sub="42 urban areas of DKI Jakarta"
	/>
	...
	<KpiCard
		...
		sub="~30.9% of residents (class 5)"
	/>
</section>

<div class="mt-8 grid gap-6 lg:grid-cols-2">
	<section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
		<h2 class="mb-4 text-lg font-semibold">Top-10 highest risk kecamatan</h2>
		<BarChart
			bars={topRisk.map((r) => ({
				label: r.kecamatan,
				value: r.risk_score,
	...
	<section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
		<h2 class="mb-4 text-lg font-semibold">Rainfall seasonality (1981-2026)</h2>
		...
		<p class="mt-2 text-xs text-slate-500">
			Mean monthly rainfall (mm) across the Jakarta study area.
		</p>
	</section>
</div>
...
{#if highest}
	<section class="mt-8 rounded-xl border border-sky-200 bg-sky-50 p-5">
		<h2 class="text-lg font-semibold text-sky-900">Priority area: {highest.kecamatan}</h2>
		...
		>
			Explore all kecamatan →
		</button>
	</section>
{/if}

<section class="mt-8">
	<h2 class="mb-3 text-lg font-semibold">All kecamatan</h2>
